...
//        get_pages_moving();
        ...
        get_destination_stock_id_combo();
        get_source_stock_id_combo();
        purchaseinvoice();
    } catch (err) {
        alert(err.message);
    }
...
function get_accountid_id_combo() {
    try {
        $('.cbo_accountid').change(function () {
            alert('The account has been selected');
...
function get_client_id_combo() {
    try {
        $('.cbo_client').change(function () {
            var cbo_client = $('.cbo_client option:selected').val();
            $('#txt_client_id').val(cbo_client);
            alert(cbo_client);
        });
    } catch (err) {
        alert(err.message);
...
function journal_entry_line_del_udpate() {
    $('.journal_entry_line_update_link').unbind('click').click(function () {
        var table_to_update = $(this).closest('td').siblings('.journal_entry_line').attr('title');
        var id_update = $(this).attr('value').trim();
        $.post('../Admin/handler.php', {id_update: id_update, table_to_update: table_to_update}, function (data) {
    ...
    $('.journal_entry_line_delete_link').unbind('click').click(function () {
        var table_to_delete = $(this).closest('td').siblings('.journal_entry_line').attr('title');
        var id_delete = $(this).attr('value').trim();
...
function get_type_project_id_combo() {//this fills the projet combobox after selcting a budget line
    try {
        $('.fly_new_p_type_project').change(function () {
            ...
            //on the fly
            if (cbo_type_project === 'fly_new_p_type_project') {
...
function get_source_stock_id_combo() {
    try {
        $('.cbo_source_stock').change(function () {
            var cbo_source_stock = $('.cbo_source_stock option:selected').val();
            $('#txt_source_stock_id').val(cbo_source_stock);
        });
    } catch (err) {
        alert(err.message);
    }
}
function get_destination_stock_id_combo() {
    try {
        $('.cbo_destination_stock').change(function () {
            var cbo_destination_stock = $('.cbo_destination_stock option:selected').val();
            $('#txt_destination_stock_id').val(cbo_destination_stock);
        });
    } catch (err) {
        alert(err.message);
    }
...
function get_items_by_stock() {
    $('.dbo_return_items_cbo').change(function () {
        try {
            ...
            $.post('../admin/handler.php', {get_items_by_stock: get_items_by_stock}, function (data) {
                alert('Here are the items: '+data);
                var final = $.parseJSON(data.trim());
                $.each(final, function (i, option) {
    ...
    $('.item_cbo_to_fill').change(function () {
        $('#txt_item_id').val($(this, 'option:changed').val());
    });
}
...
function Get_data_row_clicked() {
    $('.clickable_row').click(function () {
        ...
        var parent_w = $('.data_details_pane_load').parent().width();
        var itself = $('.data_details_pane_load').width();
        var left_space = (parent_w - itself) / 2;
        $('.data_details_pane_load').css('margin-left', left_space);
        ...
        if (unit == 'p_type_project') {
            ...
            $.post('../admin/handler.php', {details_by_click_table: details_by_click_table, id: id, p_type_project_details_click: p_type_project_details_click}, function (data) {
                ...
                $('.data_details_pane_load').fadeOut(300);
            });
        }
        if (unit == 'account') {
            var details_by_click_table = unit;
...
function save_pos() {
    $('.btn_save_po').click(function () {
        $()
    });
}

function purchaseinvoice() {
    $('.cbo_pur_invoice').change(function () {
        var purchase_invoice=$(this,'option:selected').val();
        $('#txt_pur_invoice').val(purchase_invoice);
        var res='';
        $.post('../stock/handler.php', {purchase_invoice: purchase_invoice}, function (data) {
//            $('.res').html('<a href="#" id="details_pu_invoice_link"  style="color: #000;float: right;" data-bind='+data+'>View details  </a> ');
            
            res=data;
        }).complete(function () {
            $('#txt_quantity').val(res);
            $('#txt_quantity2').val(res);
        });
       
    });  
    $('#txt_quantity').keyup(function () {
        var qty=$(this).val();
         $('#txt_quantity2').val(qty);
    });
}
